  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
        <SheetHeader>
          <SheetTitle>{isEditing ? "Edit Item" : item.name}</SheetTitle>
          <SheetDescription>
            {isEditing
            ...
            <>
              ...
              {item.blockerNotes && (
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-600">{item.blockerNotes}</p>
                </div>
              )}
              ...
              <div>
                ...
                <div className="mt-2 flex gap-2">
                  {[1, 2, 3, 4].map((q) => (
                    <div
                      key={q}
                      className={`flex h-8 w-16 items-center justify-center rounded text-sm font-medium ${
                        getQuarterNumbers().includes(q)
